...
object LocalAiRecommendationEngine {
    fun generate(uiState: DashboardUiState, currency: AppCurrency): String {
        ...
        lines += if (uiState.totalBudget > 0.0) {
            "Ta mesec si porabila ${uiState.totalSpent.formatMoney(currency)}, kar je ${budgetUsage.toInt()}% mesečnega proračuna."
        } else {
            "Ta mesec si porabila ${uiState.totalSpent.formatMoney(currency)}. Mesečni proračun še ni nastavljen, zato primerjava z limitom ni mogoča."
        }
        ...
        uiState.categorySpending
            ...
            ?.let { topCategory ->
                lines += "Največ porabe je v kategoriji ${topCategory.categoryName}: ${topCategory.spentAmount.formatMoney(currency)}."
            }
        ...
        if (overLimit.isNotEmpty()) {
            ...
            lines += "Kategorija ${category.categoryName} je presegla limit za ${overAmount.formatMoney(currency)}. Predlagam, da preveriš ta limit ali začasno zmanjšaš porabo v tej kategoriji."
            ...
            lines += "Kategorija ${category.categoryName} je blizu limita. Do limita imaš še ${remaining.formatMoney(currency)}, zato jo je smiselno spremljati."
